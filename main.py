...
_DEFAULTS = {
    ...
    "ALERT_FULL": True, "MANUAL_MAX_ALERTAS": 4, "MANUAL_SIZING": True,
    "SIGNAL_TTL_MIN": 15,
...
class Bot:
    ...
    async def start(self) -> None:
        ...
        await self.tg.send(
            ...
            + ("Polymarket: contexto ON"
               ...
               if config.PM_ENABLED else "Polymarket apagado") + chr(10)
            + (f"🎯 Partes de entrada MANUAL activos (hasta "
               f"{config.MANUAL_MAX_ALERTAS} por ciclo, caducan a los "
               f"{config.SIGNAL_TTL_MIN} min)"
               + (f" · tamaño sobre {config.SALDO_ESTIMADO:.0f} USDT"
                  if config.MANUAL_SIZING else "")
               if config.ALERT_FULL else "Avisos en formato agrupado")
        )
    ...
    def fmt_ticket(self, sig: strategy.Signal) -> str:
        """
        PARTE COMPLETO para teclear la orden a mano en BingX.

        POR QUÉ NO VALE EL AVISO NORMAL. fmt_signal() describe la señal;
        esto describe la ORDEN. La diferencia son cuatro datos que el
        otro no trae y sin los cuales hay que calcular a mano con el
        móvil en la mano y el precio moviéndose:

          · CANTIDAD para tu riesgo. Es donde más se equivoca uno: el
            tamaño NO se divide por el apalancamiento ni otra vez por el
            precio. Sale de (saldo x RISK_PCT / 100) / |entrada - SL|, y
            va redondeada a la precisión del contrato porque BingX
            rechaza los decimales de más.
          · CADUCIDAD. Una señal de hace 40 min a un precio que ya se
            movió no es la señal que se generó.
          · PLAN DE SALIDA. El bot no gestiona lo que abres tú: el
            límite de tiempo y el cruce contrario son tuyos.
          · ACIERTO DE EQUILIBRIO. Con R:R fijo hay un % de acierto por
            debajo del cual se pierde aunque todo salga "según el plan".

        La cantidad usa SALDO_ESTIMADO: en SIGNAL no hay saldo real que
        consultar. Si tu saldo cambió, cambia esa variable o el número
        es el de otra cuenta.
        """
        lado = "LARGO" if sig.side == "BUY" else "CORTO"
        base = sig.symbol.split("-")[0]
        riesgo_precio = abs(sig.entry - sig.sl)
        rr = (abs(sig.tp - sig.entry) / riesgo_precio) if riesgo_precio > 0 else 0.0
        be_wr = 100.0 / (1.0 + rr) if rr > 0 else 0.0
        limite = dt.datetime.now(dt.timezone.utc) + dt.timedelta(
            minutes=config.SIGNAL_TTL_MIN)
        tf = getattr(sig, "timeframe", config.TIMEFRAME)

        L = [
            f"🎯 <b>{lado} {base}</b> · {tf} · <i>entrada manual</i>",
            "",
            f"Entrada   <code>{sig.entry:.8g}</code>",
            f"Stop      <code>{sig.sl:.8g}</code>   ({sig.riesgo_pct:.2f}%)",
            f"Objetivo  <code>{sig.tp:.8g}</code>   ({rr:.2f} R)",
        ]

        if config.MANUAL_SIZING:
            # En LIVE puede haber patrimonio real ya cacheado por
            # snapshot(); si no, manda SALDO_ESTIMADO. No se pide aquí:
            # fmt_ticket es síncrona a propósito para que formatear un
            # mensaje nunca dispare una llamada al exchange.
            saldo = float(self._cuenta.get("equity", 0.0) or 0.0)
            if saldo <= 0:
                saldo = config.SALDO_ESTIMADO
            qty = self.api.round_qty(
                sig.symbol, strategy.position_size(saldo, sig.entry, sig.sl))
            minimo = self.api.min_qty(sig.symbol)
            nocional = qty * sig.entry
            margen = nocional / max(config.LEVERAGE, 1)
            L.append("")
            if qty <= 0 or (minimo > 0 and qty < minimo):
                riesgo_min = minimo * riesgo_precio
                pct = (riesgo_min / saldo * 100.0) if saldo > 0 else 0.0
                L.append(f"⚠️ <b>Por debajo del lote mínimo</b> ({minimo:.10g}).")
                L.append(f"Entrar al mínimo arriesgaría <b>{pct:.2f}%</b> del saldo, "
                         f"no {config.RISK_PCT}%. <i>Sáltatela o reduce a mano "
                         f"sabiendo lo que arriesgas.</i>")
            else:
                L.append(f"Cantidad  <b>{qty:.10g}</b>  ·  nocional {nocional:.2f} USDT")
                L.append(f"Margen    {margen:.2f} USDT a {config.LEVERAGE}x "
                         f"({config.MARGIN_MODE})")
                L.append(f"Riesgas   {saldo * config.RISK_PCT / 100.0:.2f} USDT "
                         f"= {config.RISK_PCT}% de {saldo:.0f}")

        L += [
            "",
            f"Coste {sig.coste_r:.2f} R · dominancia {sig.ratio:.2f} · "
            f"ER {getattr(sig, 'persist', 0):.2f} · ATR {sig.atr_pct:.2f}%",
        ]
        if getattr(sig, "funding", None) is not None:
            L.append(f"Funding {sig.funding:+.4f}%/8h — {funding.sesgo(sig.funding)}")
        if getattr(sig, "btc_24h", None) is not None:
            L.append(f"BTC {sig.btc_24h:+.1f}% 24h")

        L += [
            "",
            f"⏳ <b>Caduca a las {limite:%H:%M} UTC</b> ({config.SIGNAL_TTL_MIN} min). "
            f"Después el precio ya no es este.",
            f"🚪 Salida: objetivo, stop, o {config.MAX_TRADE_MINUTES} min — y "
            f"cierra si la tendencia cruza al revés.",
            f"⚖️ Con {rr:.2f} R necesitas acertar más del <b>{be_wr:.0f}%</b> "
            f"solo para empatar.",
            "",
            "<i>El bot NO abre ni vigila esto. Si entras, la posición es tuya: "
            "pon el SL y el TP en la misma orden.</i>",
        ]
        return chr(10).join(L)

    async def volcar_avisos(self) -> None:
        """
        Señales del ciclo que el bot no ejecutó.

        DOS FORMATOS, y el que sale depende de CUÁNTAS haya. Con pocas,
        un parte completo por señal: es lo que hace falta para operar a
        mano. Con muchas, el agrupado compacto de siempre — porque el
        modo manual no puede reinventar la fatiga de alertas que ya
        costó 1920 mensajes sin leer.
        """
        ...
        if config.ALERT_FULL and len(nuevos) <= config.MANUAL_MAX_ALERTAS:
            for sig in nuevos:
                await self.tg.send(self.fmt_ticket(sig))
            return
