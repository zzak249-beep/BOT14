"""
QF×JP Bot v7.8 — Main (renewed-love CAZADOR)
═══════════════════════════════════════════════════════════════════════════════
FIX v7.8 (esta revisión, mismos fixes que joyful-art v7.7):

  ✅ CRÍTICO: pnl_at_trail_sl ya NO multiplica por C.LEVERAGE.
     El complement engine de joyful-art y zesty lee este campo desde el
     /status de renewed-love. Con × LEVERAGE el PnL reportado era 10x
     el real → el complement bloqueaba copies válidos (creía que el master
     ya había ganado demasiado) o amplificaba señales de pérdida.

  ✅ ws_task añadido al cleanup del lifespan — antes quedaba huérfano al
     hacer Railway restart, manteniendo la conexión WS abierta durante
     varios segundos extra con mensajes a un contexto ya cerrado.

  ✅ Versión del banner actualizada a 7.8.

Sin más cambios vs v7.2.
═══════════════════════════════════════════════════════════════════════════════
"""
import asyncio
import logging
...
async def lifespan(app: FastAPI):
    ...
    log.info("═" * 54)
    log.info("  QF×JP Bot v7.8 — CAZADOR master signal")
    ...
    # FIX v7.8: ws_task guardado en variable para cancelarlo en cleanup
    ws_task = None
    if getattr(C, 'WS_ENABLED', False):
        import scanner as _scanner_mod
        ws_task = asyncio.create_task(
            run_ws_client(_scanner_mod.get_current_symbols, C.TIMEFRAME)
        )
        log.info("WS Market Data activado (WS_ENABLED=true)")

    log.info("Loops iniciados (scanner + monitor + complement + query assistant%s)",
             " + ws" if ws_task else "")

    yield

    # FIX v7.8: cancelar ws_task junto con el resto
    for task in [scanner_task, monitor_task, complement_task, query_task]:
        task.cancel()
    if ws_task:
        ws_task.cancel()
    ...
    if client:
        await client.close()
    log.info("Bot v7.8 detenido correctamente.")


app = FastAPI(
    title="QF×JP Bot v7.8 — CAZADOR",
    docs_url=None,
    redoc_url=None,
    lifespan=lifespan,
)
...
@app.get("/health")
async def health():
    return {"status": "ok", "version": "7.8", "mode": C.MODE}
    ...
    return {
        ...
        "trades":  {
            sym: {
                ...
                # FIX v7.8: quitado × C.LEVERAGE — PnL en USDT reales.
                # El complement_engine de joyful-art y zesty lee este campo.
                # Con × LEVERAGE el valor era 10-19x el real → decisiones
                # de copia incorrectas en todos los bots complementarios.
                "pnl_at_trail_sl": round(
                    (t.trail_sl - t.entry) * t.qty
                    if t.direction == "LONG" and t.trail_sl > 0
                    else (t.entry - t.trail_sl) * t.qty
