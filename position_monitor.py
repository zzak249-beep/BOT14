...
class PositionMonitor:
    ...
    async def check_closures(self, balance):
        ...
        # PAPER TRADING en DRY_RUN: las posiciones secas no existen en

        # BingX — consultarlo las daria por cerradas al instante con PnL

        # basura. Se cierran contra velas reales (misma vela SL y TP = SL).
    ...
    async def _paper_check_closures(self, balance):
        ...
        for symbol in list(self.tracked.keys()):
            ...
            try:
                ...
                kl = await self.client.get_klines(symbol, config.ENTRY_TF, limit=250)

            except Exception as e:

                log.warning("[%s] paper: sin velas para evaluar cierre (%s)", symbol, e)
            ...
            self.journal.record({
                ...
                "held_min": round((_t.time() * 1000 - meta.get("opened_at_ms", 0)) / 60000),
